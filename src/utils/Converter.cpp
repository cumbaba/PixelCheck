...
QImage Converter::cvMatToQImage(const cv::Mat& inMat) {
    switch (inMat.type()) {
        // 8-bit, 4 channel
        case CV_8UC4: {
...
QPixmap Converter::cvMatToQPixmap(const cv::Mat& inMat) {
    return QPixmap::fromImage(cvMatToQImage(inMat));
}

cv::Mat Converter::QImageToCvMat(const QImage& inImage, bool inCloneImageData) {
